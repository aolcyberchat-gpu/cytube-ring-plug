 ...
 * ================================================================
 *  VERSION HISTORY
 *  v0.9  2026-08-13  on-demand sockets: connect only on tooltip
 *                    hover, disconnect on close — fixes 429 rate
 *                    limit errors from persistent polling connections.
 *                    Use io() (no URL) to reuse CyTube's own server.
 *  v0.8  2026-08-12  (prior persistent socket approach — caused 429s)
 * ================================================================
 */
...
(function () {
    ...
    var liveData = {};
    // liveData[roomName] = { users: '?', nowPlaying: null, _socket: null }

    /*
     * ON-DEMAND SPY SOCKET
     * Only connects when a tooltip is hovered. Disconnects when tooltip closes.
     * This avoids the 429 rate limit from persistent polling connections.
     *
     * From CyTube's ui.js: the global socket object is `socket` and it
     * connects via io() to the same server as the current page.
     * We use io() with no URL to reuse CyTube's socket.io server correctly.
     */
    function connectSpy(roomName, onUpdate) {
        if (!window.io) return; // socket.io not ready — tooltip shows static data
        var d = liveData[roomName];
        if (d && d._socket) return; // already connected for this room

        liveData[roomName] = liveData[roomName] || { users: '?', nowPlaying: null, _socket: null };
        var ld = liveData[roomName];

        try {
            var sock = io({ forceNew: true, reconnection: false });
            ...
            sock.on('userlist', function (users) {
                ld.users = users.length;
                onUpdate();
            });
            sock.on('addUser', function () {
                ld.users = (typeof ld.users === 'number' ? ld.users : 0) + 1;
                onUpdate();
            });
            sock.on('userLeave', function () {
                ld.users = Math.max(0, (typeof ld.users === 'number' ? ld.users : 1) - 1);
                onUpdate();
            });
            sock.on('changeMedia', function (data) {
                ld.nowPlaying = data.title || null;
                onUpdate();
            });
    ...
    function disconnectSpy(roomName) {
        var d = liveData[roomName];
        if (d && d._socket) {
            try { d._socket.disconnect(); } catch(e) {}
            d._socket = null;
        }
    }
    ...
    function buildBadge(ring, idx) {
        ...
        function makeBtn(icon2, label, href, tip, disabled, spyRoom) {
            ...
            // On-demand spy: connect when tooltip hovered, disconnect when done
            if (spyRoom) {
                btn.addEventListener('mouseenter', function () {
                    connectSpy(spyRoom, function () {
                        // Rebuild this button's tooltip with fresh data
                        var tipEl = btn.querySelector('.crp-tip');
                        if (!tipEl) return;
                        var isNext = label === 'Next';
                        var tmp = document.createElement('div');
                        tmp.innerHTML = buildTip(isNext ? next : prev, isNext ? '→' : '←');
                        var built = tmp.firstChild;
                        if (built) btn.replaceChild(built, tipEl);
                    });
                });
                btn.addEventListener('mouseleave', function () {
                    disconnectSpy(spyRoom);
                });
            }
            nav.appendChild(btn);
        }

        makeBtn('\u25C4', 'Prev', prev ? prev.url : null, buildTip(prev, '\u2190'), !prev, prev ? prev.name : null);
        makeBtn('\uD83C\uDFE0', 'Home', homeUrl, '', false, null);
        makeBtn('\u25BA', 'Next', next ? next.url : null, buildTip(next, '\u2192'), !next, next ? next.name : null);
    ...
    function init() {
        ...
        fetch(cfg.registryUrl)
            ...
            .then(function (ring) {
                ...
                buildBadge(ring, idx);
                // Spy sockets are now on-demand (hover only) — no persistent connections
