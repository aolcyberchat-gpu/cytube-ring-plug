 ...
 *                    (was silently bailing on race with CyTube);
 *                    queue event for initial now-playing state;
 *                    mailto: temp anchor fix for Android/Vivaldi;
 *                    tooltip live-rebuild fixed (replaceChild)
 *  v0.3  2026-08-11  registryUrl switched to raw GitHub (no CDN
 *                    cache on ring.json — updates instantly);
 *                    ring.json fetched via fetch() so raw GitHub
 *                    MIME is not an issue for JSON data
...
(function () {
    ...
    /* ============================================================
       SPY SOCKET — connect to a room as a read-only observer
       Retries until window.io is available (race with CyTube init)
       ============================================================ */
    function openSpy(roomName, onUpdate) {
        // Retry until socket.io is ready — CyTube loads it async
        if (!window.io) {
        ...
        try {
            ...
            sock.on('queue', function (data) {
                // initial queue dump on join — grab first item if nothing playing
                if (!liveData[roomName].nowPlaying && data && data.length) {
                    liveData[roomName].nowPlaying = data[0].media && data[0].media.title || null;
                    onUpdate(roomName);
                }
            });
    ...
    function buildBadge(ring, idx) {
            ...
            if (href && !disabled) {
                ...
                btn.addEventListener('click', function (e) {
                    ...
                    if (href.indexOf('mailto:') === 0) {
                        // Android-safe mailto: create temp anchor and click it
        ...
        // Rebuild tooltips live as spy socket data arrives
        window._crpRefreshTips = function () {
            var btns = nav.querySelectorAll('.crp-btn');
            // btn[0]=Prev, btn[2]=Next
            [{ btn: btns[0], room: prev, label: '\u2190' },
             { btn: btns[2], room: next, label: '\u2192' }
            ].forEach(function (item) {
                if (!item.btn || !item.room) return;
                var oldTip = item.btn.querySelector('.crp-tip');
                var newTip = document.createElement('div');
                newTip.outerHTML; // force parse
                var tmp = document.createElement('div');
                tmp.innerHTML = buildTip(item.room, item.label);
                var built = tmp.firstChild;
                if (oldTip) {
                    item.btn.replaceChild(built, oldTip);
                } else {
                    item.btn.appendChild(built);
                }
            });
        };
    }
