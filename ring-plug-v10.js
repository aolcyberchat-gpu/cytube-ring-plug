 ...
 * ================================================================
 *  VERSION HISTORY
 *  v1.0  2026-08-13  removed spy sockets entirely — all socket
 *                    approaches caused CyTube socket init errors on
 *                    navigation. Tooltips now show static ring.json
 *                    data (name, vibe, owner). Clean, zero errors.
 *  v0.9  2026-08-13  on-demand sockets (still caused nav errors)
 *  v0.8  2026-08-12  persistent sockets (caused 429s)
 * ================================================================
 */
...
(function () {
    ...
    /*
     * Live room data (user count, now playing) via spy sockets caused
     * CyTube socket init errors on navigation. Tooltips now show static
     * data from ring.json — reliable, zero errors.
     * Live data is a TODO for when a server-side proxy is available.
     */
    ...
    function buildTip(room, label) {
        ...
        return [
            ...
            room.owner ? '<div class="crp-tip-meta">\uD83D\uDC64 ' + room.owner + '</div>' : '',
            room.vibe  ? '<div class="crp-tip-meta">\uD83C\uDF10 ' + room.vibe + '</div>'  : '',
            '</div>'
        ].join('');
    }
    ...
    function init() {
        ...
        fetch(cfg.registryUrl)
            ...
            .then(function (ring) {
                ...
                window.CRP = { ring: ring, idx: idx, cfg: cfg };
            })
